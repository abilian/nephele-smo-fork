...
class PrometheusHelper():
    ...
    def get_latency(self, name):
        ...
        prometheus_latency_metric_name = '(sum(rate(flask_http_request_duration_seconds_sum'\
            '{{service="{0}"}}[{1}{2}])) by (service))'\
            '/(sum(rate(flask_http_request_duration_seconds_count'\
            '{{service="{0}"}}[{1}{2}])) by (service))'.format(name, self.time_window, self.time_unit)
    ...
    def get_cpu_util(self, name):
        ...
        cpu_util_metric_name = 'round(100 *sum(rate(container_cpu_usage_seconds_total'\
        ...
        # Get cpu_util
        cpu_util = self.query_prometheus(
            self.prometheus_host,
